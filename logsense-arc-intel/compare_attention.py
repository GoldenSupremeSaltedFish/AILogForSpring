...
class AttentionComparisonTrainer:
    ...
    def compare_models(self, data_path: str, epochs: int = 10):
        ...
        # 训练带Attention的TextCNN
        logger.info("🔍 训练TextCNN with Attention")
        attention_model = TextCNNWithAttention(**model_config, attention_dim=128)
        attention_acc, attention_f1 = self.train_model(
            attention_model, train_loader, val_loader, epochs, "TextCNN with Attention"
        )
        
        # 对比结果
        logger.info("📊 对比结果:")
        logger.info(f"  原始TextCNN: 准确率={original_acc:.2f}%, F1={original_f1:.4f}")
        logger.info(f"  TextCNN+Attention: 准确率={attention_acc:.2f}%, F1={attention_f1:.4f}")
        
        acc_improvement = attention_acc - original_acc
        f1_improvement = attention_f1 - original_f1
        ...
        if acc_improvement > 0 and f1_improvement > 0:
            logger.info("✅ Attention机制有效提升了模型性能!")
        else:
...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description='Attention对比训练')
