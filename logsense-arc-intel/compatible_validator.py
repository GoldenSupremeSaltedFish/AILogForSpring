...
class CompatibleValidator:
    ...
    def __init__(self, model_path, data_path):
        ...
        logger.info(f"🚀 初始化兼容性验证器，使用设备: {self.device}")
    ...
    def _rebuild_model_from_checkpoint(self, checkpoint):
        ...
        try:
            ...
            # 创建兼容的模型结构
            class CompatibleFeatureModel(nn.Module):
            ...
            # 创建模型实例
            model = CompatibleFeatureModel(model_state_dict)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="兼容性模型验证器")
