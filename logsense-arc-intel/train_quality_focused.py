...
class QualityFocusedDataset(torch.utils.data.Dataset):
    """基于数据质量的数据集"""
...
class QualityFocusedTrainer:
    ...
    def load_enhanced_data(self, data_path: str):
        """加载增强数据"""
        logger.info(f"📂 加载增强数据: {data_path}")
        df = pd.read_csv(data_path)
        logger.info(f"📊 增强数据: {len(df)} 条记录")
    ...
    def train_model(self, model, train_loader, val_loader, epochs=15, model_name="QualityFocusedModel"):
        """训练模型"""
    ...
    def train_quality_focused(self, data_path: str, epochs: int = 15):
        """基于数据质量的训练"""
        logger.info("🎯 基于数据质量的训练")
        
        # 加载增强数据
        texts, labels, label_encoder, df_enhanced = self.load_enhanced_data(data_path)
        ...
        # 创建数据集
        train_dataset = QualityFocusedDataset(train_texts, train_labels)
        val_dataset = QualityFocusedDataset(val_texts, val_labels)
        ...
        # 训练模型
        logger.info("🔍 训练基于数据质量的TextCNN")
        model = TextCNN(**model_config)
        best_acc, best_f1, history = self.train_model(
            model, train_loader, val_loader, epochs, "QualityFocusedTextCNN"
        )
        ...
        # 保存结果
        self._save_results(model, label_encoder, history, best_acc, best_f1, df_enhanced)
        
        return model, label_encoder, history
    
    def _save_results(self, model, label_encoder, history, best_acc, best_f1, df_enhanced):
        """保存训练结果"""
        # 创建时间戳
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        
        # 保存模型
        model_path = f"results/models/quality_focused_model_{timestamp}.pth"
        ...
        # 保存训练历史
        history_path = f"results/history/quality_focused_history_{timestamp}.json"
    ...
    def _plot_training_curves(self, history, timestamp):
        ...
        # 保存图片
        plot_path = f"results/plots/quality_focused_training_{timestamp}.png"
...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description='基于数据质量的训练')
    parser.add_argument('--data', type=str, default='data/processed_logs_quality_enhanced.csv',
                       help='增强数据文件路径')
    ...
    model, label_encoder, history = trainer.train_quality_focused(args.data, args.epochs)
    
    logger.info("✅ 基于数据质量的训练完成!")
