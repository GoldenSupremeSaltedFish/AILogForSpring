...
class CompleteValidator:
    ...
    def __init__(self, model_path, data_path):
        ...
        logger.info(f"🚀 初始化完整验证器，使用设备: {self.device}")
    ...
    def load_model_and_components(self):
        ...
        try:
            ...
            # 重新构建模型
            self.model = self._rebuild_feature_enhanced_model(checkpoint)
            self.model.eval()
    ...
    def _rebuild_feature_enhanced_model(self, checkpoint):
        """重新构建特征增强模型"""
        try:
            # 从checkpoint中获取模型结构信息
    ...
    def create_visualizations(self, validation_results, stats, category_analysis):
        ...
        try:
            ...
            plt.bar(categories, category_counts, color='lightcoral', alpha=0.7)
            plt.xlabel('类别')
            plt.ylabel('样本数量')
            plt.title('各类别样本数量')
            plt.xticks(rotation=45, ha='right')
...
def main():
    ...
    parser = argparse.ArgumentParser(description="完整模型验证器")
