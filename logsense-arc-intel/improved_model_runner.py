...
class ImprovedModelRunner:
    ...
    def __init__(self, model_path, data_path):
        ...
        # 创建结果目录
        self.results_dir = "improved_validation_results"
        ...
        logger.info(f"🚀 初始化改进的模型运行器，使用设备: {self.device}")
    ...
    def save_results(self, result):
        ...
        try:
            ...
            # 保存验证结果
            results_file = os.path.join(self.results_dir, f"improved_validation_results_{timestamp}.json")
    ...
    def save_detailed_report(self, result, timestamp):
        """保存详细报告"""
        try:
            report_file = os.path.join(self.results_dir, f"improved_detailed_report_{timestamp}.txt")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="改进的模型运行器")
