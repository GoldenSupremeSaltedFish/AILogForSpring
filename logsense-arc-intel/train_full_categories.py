...
class FullCategoryDataset(torch.utils.data.Dataset):
    """完整类别数据集"""
...
class FullCategoryTrainer:
    ...
    def __init__(self, model_type: str = "textcnn"):
        ...
        logger.info(f"�� 初始化完整类别训练器 - 模型类型: {model_type}")
    ...
    def create_model(self, num_classes: int):
        ...
        model_config = {
            'vocab_size': 8000,  # 增加词汇表大小
            'embed_dim': 128,     # 增加嵌入维度
            'num_classes': num_classes,
            'filter_sizes': [3, 4, 5],
            'num_filters': 128,   # 增加卷积核数量
            'dropout': 0.5        # 适中的Dropout
        }
    ...
    def train(self, data_path: str, epochs: int = 30, patience: int = 8, batch_size: int = 32):
        ...
        # 创建数据集
        train_dataset = FullCategoryDataset(train_texts, train_labels)
        val_dataset = FullCategoryDataset(val_texts, val_labels)
        test_dataset = FullCategoryDataset(test_texts, test_labels)
        ...
        logger.info(f"✅ 完整类别训练完成 - 最佳F1: {best_f1:.4f}, 最佳准确率: {best_acc:.2f}%")
...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description="完整类别Intel Arc GPU 训练器")
    parser.add_argument("--model", type=str, default="textcnn",
                       choices=["textcnn", "fasttext"], help="模型类型")
    parser.add_argument("--data", type=str, default="data/processed_logs_full.csv", 
                       help="数据文件路径")
    ...
    logger.info("�� 完整类别Intel Arc GPU 训练器")

    trainer = FullCategoryTrainer(model_type=args.model)
