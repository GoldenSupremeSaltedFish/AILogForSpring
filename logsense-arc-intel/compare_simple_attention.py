...
from models.textcnn_simple_attention import TextCNNSimpleAttention
...
class SimpleAttentionComparisonTrainer:
    """简化Attention对比训练器"""
    ...
    def load_data(self, data_path: str):
        ...
        texts = df_cleaned['original_log'].tolist()
        
        return texts, labels, label_encoder
    
    def train_model(self, model, train_loader, val_loader, epochs=15, model_name="Model"):
        """训练模型"""
    ...
    def compare_models(self, data_path: str, epochs: int = 15):
        """对比模型"""
        logger.info("🎯 简化Attention对比训练")
        ...
        # 训练简化Attention TextCNN
        logger.info("🔍 训练TextCNN with Simple Attention")
        simple_attention_model = TextCNNSimpleAttention(**model_config)
        simple_acc, simple_f1 = self.train_model(
            simple_attention_model, train_loader, val_loader, epochs, "TextCNN with Simple Attention"
        )
        results['simple_attention'] = {'acc': simple_acc, 'f1': simple_f1}
        
        # 对比结果
        logger.info("📊 对比结果:")
        logger.info(f"  原始TextCNN: 准确率={original_acc:.2f}%, F1={original_f1:.4f}")
        logger.info(f"  简化Attention: 准确率={simple_acc:.2f}%, F1={simple_f1:.4f}")
        
        # 计算改进幅度
        acc_improvement = simple_acc - original_acc
        f1_improvement = simple_f1 - original_f1
        
        logger.info(f"📈 改进幅度:")
        logger.info(f"  准确率提升: {acc_improvement:.2f}%")
        logger.info(f"  F1分数提升: {f1_improvement:.4f}")
        
        # 判断最佳模型
        best_model = max(results.items(), key=lambda x: x[1]['f1'])
        logger.info(f"🏆 最佳模型: {best_model[0]} (F1: {best_model[1]['f1']:.4f})")
        
        if simple_f1 > original_f1:
            logger.info("✅ 简化Attention机制有效提升了模型性能!")
        else:
...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description='简化Attention对比训练')
    parser.add_argument('--data', type=str, default='data/processed_logs_balanced_attention.csv',
                       help='数据文件路径')
    parser.add_argument('--epochs', type=int, default=15, help='训练轮数')
    
    args = parser.parse_args()
    
    trainer = SimpleAttentionComparisonTrainer()
