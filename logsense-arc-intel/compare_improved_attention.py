...
from models.textcnn_improved_attention import TextCNNImprovedAttention
...
class ImprovedAttentionComparisonTrainer:
    """改进的Attention对比训练器"""
    ...
    def compare_models(self, data_path: str, epochs: int = 15):
        ...
        # 训练改进的Attention TextCNN
        logger.info("🔍 训练TextCNN with Improved Attention")
        improved_attention_model = TextCNNImprovedAttention(**model_config, num_heads=4)
        improved_acc, improved_f1 = self.train_model(
            improved_attention_model, train_loader, val_loader, epochs, "TextCNN with Improved Attention"
        )
        results['improved_attention'] = {'acc': improved_acc, 'f1': improved_f1}
        
        # 对比结果
        logger.info("📊 对比结果:")
        logger.info(f"  原始TextCNN: 准确率={original_acc:.2f}%, F1={original_f1:.4f}")
        logger.info(f"  改进Attention: 准确率={improved_acc:.2f}%, F1={improved_f1:.4f}")
        
        # 计算改进幅度
        acc_improvement = improved_acc - original_acc
        f1_improvement = improved_f1 - original_f1
        ...
        if improved_f1 > original_f1:
            logger.info("✅ 改进的Attention机制有效提升了模型性能!")
        else:
...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description='改进的Attention对比训练')
    ...
    trainer = ImprovedAttentionComparisonTrainer()
