...
# -*- coding: utf-8 -*-
"""
简化的Intel Arc GPU 训练脚本 - 使用解耦的持久化系统
"""
...
class SimpleLogDataset(torch.utils.data.Dataset):
    """简化的日志数据集"""
...
class SimpleArcTrainer:
    """简化的Intel Arc GPU 训练器"""
    ...
    def __init__(self, model_type: str = "textcnn"):
        ...
        logger.info(f" 初始化简化训练器 - 模型类型: {model_type}")
    ...
    def train(self, data_path: str, epochs: int = 20, patience: int = 5, batch_size: int = 16):
        """训练流程"""
        logger.info("🚀 开始简化训练")
        ...
        # 创建数据集
        train_dataset = SimpleLogDataset(train_texts, train_labels)
        val_dataset = SimpleLogDataset(val_texts, val_labels)
        test_dataset = SimpleLogDataset(test_texts, test_labels)
        ...
        logger.info(f"✅ 简化训练完成 - 最佳F1: {best_f1:.4f}, 最佳准确率: {best_acc:.2f}%")
    ...
    def _save_all_results(self, best_acc, best_f1, test_acc, test_f1, final_epoch,
        ...
        self.persistence.save_confusion_matrix(best_labels, best_predictions, class_names)

        # 保存分类报告
        self.persistence.save_classification_report(best_labels, best_predictions, class_names)
...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description="简化的Intel Arc GPU 训练器")
    ...
    logger.info("🎯 简化的Intel Arc GPU 训练器")

    trainer = SimpleArcTrainer(model_type=args.model)
